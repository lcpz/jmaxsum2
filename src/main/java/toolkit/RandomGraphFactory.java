/*
Taken from FRODO 2.16 (frodo2.algorithms.RandGraphFactory)
...
public class RandomGraphFactory {
	...
	public static class Graph {
		...
		public Map<String, Set<String>> neighbourhoods;
		 ...
		 * @param neighbourhoods for each node, the list of its neighbours
		 * @param clusters       list of node clusters
		 */
		public Graph(List<String> nodes, Edge[] edges, List<List<String>> components,
				Map<String, Set<String>> neighbourhoods, List<List<String>> clusters) {
			this.nodes = nodes;
			this.edges = edges;
			...
			if (clusters == null) {
				...
				for (int i = 0; i < clusters.size(); i++) {
					...
					for (String node : nodes) {
						...
						// fill this node if connected to at least one node inside a different cluster
						for (String neigh : this.neighbourhoods.get(node))
		...
		/**
		 * @return the p1 density, computed as the number of edges divided by the
		...
		/** @return the highest degree (i.e. number of neighbours) of any given node */
		public int computeMaxDeg() {
			int maxDeg = 0;
			for (Set<String> neighbours : this.neighbourhoods.values())
				maxDeg = Math.max(maxDeg, neighbours.size());
			return maxDeg;
		}
	}
	...
	public static Graph getSizedRandGraph(int nbrNodes, int nbrEdges, int nbrClusters) {
		...
		if (nbrEdges > maxNbrEdges) {
			System.err.println("Cannot create a graph with " + nbrEdges + " edges and only " + nbrNodes
					+ " nodes; truncating to " + maxNbrEdges + " edges");
		...
		// Keep track of the connected components of the graph; initially one component
		// per node
		...
		// Keep track of neighbour relationships
		Map<String, Set<String>> neighbours = new HashMap<String, Set<String>>(nbrNodes);
		for (String node : nodes)
			neighbours.put(node, new HashSet<String>());
		...
		for (int i = 0; i < nbrEdges; i++) {
			...
			neighbours.get(i2Name).add(i1Name);
			neighbours.get(i1Name).add(i2Name);
		...
		if (nbrClusters == 0) // we don't want any cluster
			return new Graph(nodes, edges, components, neighbours, null);
		else {
			...
			return new Graph(nodes, edges, components, neighbours, clusters);
		}
	}
	 ...
	 *
	 * The graph contains at least 2 nodes. There can be only at most 1 edge between
	 * any two given nodes.
	 *
	 * @param maxNbrNodes    maximum number of nodes. Must be at least 2.
	 * @param maxNbrEdges    maximum number of edges
	 * @param maxNbrClusters maximum number of clusters; if 0, no clusters are
	 *                       created
	 * @return a random graph
	 */
	...
	/**
	 * Generates a random graph in which, for each cluster, all variables in that
	 * cluster belong to the same graph component
	 *
	 ...
	 * @return a random graph in which, for each cluster, all variables in that
	 *         cluster belong to the same graph component
	 */
	public static Graph getNiceRandGraph(int maxNbrNodes, int maxNbrEdges, int maxNbrClusters) {
		...
		while (wrong) {
			...
			graph = RandomGraphFactory.getRandGraph(maxNbrNodes, maxNbrEdges, maxNbrClusters);
	...
	public static Graph getRingGraph(int nbrNodes) {
		...
		HashMap<String, Set<String>> neighbourhoods = new HashMap<String, Set<String>>(nbrNodes);
		for (int i = 1; i <= nbrNodes; i++)
			neighbourhoods.put(Integer.toString(i % nbrNodes), new HashSet<String>(
					Arrays.asList(Integer.toString((i - 1) % nbrNodes), Integer.toString((i + 1) % nbrNodes))));

		return new Graph(nodes, edges, components, neighbourhoods, null);
	}
	...
	public static Graph getSquareGrid(int side) {
		...
		// Build the neighbourhoods
		HashMap<String, Set<String>> neighbourhoods = new HashMap<String, Set<String>>(side * side);
		for (int i = 0; i < side; i++) {
			for (int j = 0; j < side; j++) {
				HashSet<String> neighbours = new HashSet<String>();
				neighbourhoods.put("n" + i + "_" + j, neighbours);

				if (i > 0)
					neighbours.add("n" + (i - 1) + "_" + j);

				if (i < side - 1)
					neighbours.add("n" + (i + 1) + "_" + j);

				if (j > 0)
					neighbours.add("n" + i + "_" + (j - 1));
		...
		return new Graph(nodes, edges.toArray(new Edge[nbrEdges]), components, neighbourhoods, null);
	}

	/**
	 * Generates a single-component, acyclic graph of the desired size, with the
	 * desired branching factor
	 *
	 * @param nbrNodes        the number of nodes
	 * @param branchingFactor each node has at most (branchingFactor + 1) neighbours
	 * @return the graph
	 */
	public static Graph getAcyclicGraph(int nbrNodes, int branchingFactor) {
		...
		HashMap<String, Set<String>> neighbourhoods = new HashMap<String, Set<String>>(nbrNodes);
		for (int i = 0; i < nbrNodes; i++)
			neighbourhoods.put(Integer.toString(i), new HashSet<String>());

		// Generate the edges and neighbourhoods
		ArrayList<Edge> edges = new ArrayList<Edge>(nbrNodes - 1); // each node has an edge with its parent, except the
																	// root
		...
		while (!open.isEmpty()) {
			...
			for (int i = 0; i < branchingFactor && !open.isEmpty(); i++) {
				...
				neighbourhoods.get(leaf).add(child);
				neighbourhoods.get(child).add(leaf);
			}
		}
		...
		return new Graph(nodes, edges.toArray(new Edge[edges.size()]), components, neighbourhoods, clusters);
	}
	...
	public static Graph getChordalGraph(int nbrNodes, double rateOfChords) {
		...
		if (nbrNodes + nbrChords > nbrNodes * (nbrNodes - 1) / 2) {
			...
			System.err.println("The rate of chords is too high; generating a complete graph with a rate of chords of "
					+ (nbrChords / (nbrChords + nbrNodes)));
		}
		...
		for (int n = 0; n < nbrChords; n++) {
			// Pick a random pair of nodes that are not consecutive and are not already
			// connected by a chord
			int i, j;
			do {
				i = rand.nextInt(nbrNodes);
				j = (i + 2 + rand.nextInt(nbrNodes - 3)) % nbrNodes;
			} while (graph.neighbourhoods.get(Integer.toString(i)).contains(Integer.toString(j)));

			// Add a chord between i and j
			graph.neighbourhoods.get(Integer.toString(i)).add(Integer.toString(j));
			graph.neighbourhoods.get(Integer.toString(j)).add(Integer.toString(i));
			chords.add(new Edge(Integer.toString(i), Integer.toString(j)));
		}

		chords.addAll(Arrays.asList(graph.edges));
		return graph = new Graph(graph.nodes, chords.toArray(new Edge[(int) nbrChords + nbrNodes]), graph.components,
				graph.neighbourhoods, null);
	}
